<template>
  <div class="relative">
    ...
    <div class="relative">
      <input
        v-model="searchQuery"
        :placeholder="selectedUnits ? selectedUnits.name : 'Search units...'"
    ...
    <!-- Dropdown -->
    <div
      v-if="showDropdown && filteredUnits.length > 0"
      ...
      <div
        v-for="(units, index) in filteredUnits"
        :key="units.id"
        :class="[
        ...
        @mousedown.prevent="selectUnits(units)">
        <span :class="['block truncate', isSelected(units) && 'font-semibold']">
          {{ units.name }}
        </span>
        <span
          v-if="isSelected(units)"
          :class="[
...
const props = defineProps({
  ...
  const selectedUnits = computed(() => {
    ...
    return props.billingUnits.find((u) => u.id === id);
  });

  const filteredUnits = computed(() => {
    if (!searchQuery.value) return props.billingUnits;
    const query = searchQuery.value.toLowerCase();
    return props.billingUnits.filter((u) => u.name.toLowerCase().includes(query));
  });

  const isSelected = (units) => {
    if (!props.modelValue) return false;
    const currentId =
      typeof props.modelValue === 'object' ? props.modelValue.id : props.modelValue;
    return units.id === currentId;
  };
  ...
  const selectFirst = () => {
    if (filteredUnits.value.length > 0) {
      selectUnits(filteredUnits.value[0]);
    }
  };
