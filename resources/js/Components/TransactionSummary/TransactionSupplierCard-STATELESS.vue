    ...
    <dl class="-my-3 divide-y divide-gray-100 px-6 py-4 text-sm leading-6">
      ...
      <div class="flex justify-between gap-x-4 py-3">
        <dd class="flex items-start gap-x-2">
          <div>
            <Combobox
              :model-value="getCurrentSupplierId()"
              @update:model-value="debouncedHandleChange">
...
const props = defineProps({
  ...
  // NO LOCAL STATE - ZERO REACTIVE VARIABLES
  // Everything reads directly from props
  ...
  // Display value for ComboboxInput - directly from props
  const getDisplayValue = (id) => {
    const supplierId = id || getCurrentSupplierId();
    const supplier = findSupplierById(supplierId);
    return supplier ? supplier.last_legal_name : '';
  };

  // Debounced handler - NEVER mutates any local state
  let pendingUpdate = false;
  let debounceTimer = null;

  const debouncedHandleChange = (newId) => {
    if (pendingUpdate) return; // Block if already processing
