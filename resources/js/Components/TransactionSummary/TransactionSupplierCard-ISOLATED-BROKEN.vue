...
const props = defineProps({
  ...
  // Completely isolated state - NO reactive dependencies on props
  ...
  onMounted(() => {
    ...
    // Sync with parent periodically (no watchers = no reactive loops)
  ...
  // Handle user selection - completely async to break reactive cycle
  const handleSupplierChange = (newId) => {
    ...
    // Use setTimeout instead of nextTick to completely break from Vue's reactive system
    setTimeout(() => {
      ...
      // Reset flag after a delay to ensure parent has processed the update
      setTimeout(() => {
