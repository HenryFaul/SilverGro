...
const form = useForm({
  ...
  transport_finance_id:
    props.transport_trans_id == null ? null : props.transport_finance_id,
  assigned_user_supplier_id:
    props.assigned_user_comm == null
      ? 1
      : props.assigned_user_comm.assigned_user_supplier_id,
  assigned_user_customer_id:
    props.assigned_user_comm == null
      ? 1
      : props.assigned_user_comm.assigned_user_customer_id,
  notes:
    props.assigned_user_comm == null ? null : props.assigned_user_comm.notes,
});

const deleteAssignedComm = () => {
  if (confirm('Sure you want to delete?')) {
    form.delete(
      route('assigned_user_comm.destroy', props.assigned_user_comm.id),
      {
        preserveScroll: true,
        onSuccess: () => {
          close();
...
const updateAssignedComm = async () => {
  await form.put(
    route('assigned_user_comm.update', props.assigned_user_comm.id),
    {
      preserveScroll: true,
      onSuccess: () => {
        close();
            ...
            <form class="w-full">
              ...
              <div :class="borderClass">
                <div class="text-lg mb-2 text-indigo-400">
                  Assigned User Commission
                </div>

                <div class="mt-3">
                  <label
                    class="block text-sm font-medium leading-6 text-gray-900"
                    >Assigned Supplier Staff</label
                  >

                  <select
                    v-model="form.assigned_user_supplier_id"
                    class="mt-2 block w-full rounded-md border-0 py-1.5 pl-3 pr-10 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-indigo-600 sm:text-sm sm:leading-6"
                  >
                    <option
                      v-for="n in props.all_staff"
                      :key="n.id"
                      :value="n.id"
                    >
                  ...
                  <InputError
                    class="mt-2"
                    :message="form.errors.assigned_user_supplier_id"
                  />
                </div>
                ...
                <div class="mt-3">
                  <label
                    class="block text-sm font-medium leading-6 text-gray-900"
                    >Assigned Customer Staff</label
                  >

                  <select
                    v-model="form.assigned_user_customer_id"
                    class="mt-2 block w-full rounded-md border-0 py-1.5 pl-3 pr-10 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-indigo-600 sm:text-sm sm:leading-6"
                  >
                    <option
                      v-for="n in props.all_staff"
                  ...
                  <InputError
                    class="mt-2"
                    :message="form.errors.assigned_user_customer_id"
                  />
                </div>
